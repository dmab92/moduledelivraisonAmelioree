...
class Picking(models.Model):
    ...
    codesms1= fields.Integer(string="Code du sms recu du client",default=1)
    codesms2= fields.Integer(string="Code a envoyer au client", default=0, readonly=True)
    mobile_phone = fields.Char(string='Telephone', related='partner_id.phone',readonly=True)
    ...
    # Ajout Boris
    # def get_code_send_sms(self):
    #     """ Cette fonction genere un code aleatoire de
    #      6 chiffres dans la variable codesms2
    #             """
    #     for picking in self:
    #         picking.codesms2 = str(random.randint(100000, 999999))



    def get_code_send_sms(self):
        """  l'envoi par  sms au numero indiquer dans le le champ mobile
        """
        for picking in self:
            picking.codesms2 = str(random.randint(100000, 999999))
        articles = []
        ...
        for line in self.move_ids_without_package:
            articles.append(line.name)
            quantites.append(line.product_uom_qty)
        ...
        message = "M./Mme " + self.partner_id.name + " votre commande numero " + str(self.origin) + " contenant "+str(int(quantites[0]))+" " +str(articles[0])+ " a comme code " \
                  + str(self.codesms2) + ". Bien vouloir le communiquer a votre livreur SOACAM  pour confirmation de votre livraison"
        sms_body = {
            'action': 'send-sms',
            'api_key': api_key,
            'to': destination,
    ...
    def button_validate(self):
        ...
        if self.codesms2 != self.codesms1:
            raise UserError(_("Code incorrect, Veillez entrez le code de 6 caracteres recu par SMS par le client."))
        self.ensure_one()
        ...
        if self.digital_signature:
            ...
            senderID = 'SOACAM'
            destination = str(self.mobile_phone)
            message = "M./Mme " + self.partner_id.name + " votre commande numero " + str(self.origin) + " contenant "+str(int(quantites[0]))+" " +str(articles[0])+ " viens d'etre livrée " \
                    ".Pour plus d'information veillez contacter SOACAM via le 655546666. Merci de  faire confiance a SOACAM."
            sms_body = {
                'action': 'send-sms',
                'api_key': api_key,
                'to': destination,
